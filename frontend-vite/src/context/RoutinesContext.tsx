...
import { Routine, Workout } from "../utils/types";
...
type ACTIONTYPE =
    ...
    | {type: "DELETE_WORKOUT"; payload: {routineId: string, workout: Workout}};
...
const routinesReducer = (prevState: RoutineContextType, action: ACTIONTYPE) => {
    ...
    switch (action.type) {
        ...
        case "UPDATE_ROUTINE":
            return {
                routines: prevState.routines.map(r => r.id === action.payload.id ? action.payload : r)
            };
        case "ADD_WORKOUT":
            return {
            ...
            return {
                routines: prevState.routines.map(r => {
                    if(r.id === action.payload.routineId) {
                        return {
                            ...r,
                            workouts: r.workouts.map(w => w.id === action.payload.workout.id ? action.payload.workout : w)
                        }
                    }
                    return r;
                })
            ...
            return {
                routines: prevState.routines.map(r => {
                    if(r.id === action.payload.routineId) {
                        return {
                            ...r,
                            workouts: r.workouts.filter(w => w.id !== action.payload.workout.id)
                        }
                    }
                    return r;
                })
...
export const RoutinesContextProvider = ({ children }: {children: JSX.Element}) => {
